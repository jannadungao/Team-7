  ...
  return (
    <Listbox value={value} onChange={(selected) => onSelect?.(selected)}>
      <div className="relative mt-2">
        <ListboxButton className="flex cursor-pointer rounded-md py-1.5 pr-2 pl-3 text-left text-white outline-1 -outline-offset-1 outline-white/10 focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-indigo-500 sm:text-sm/6">
        ...
        <ListboxOptions
          transition
          className="mt-1 fixed z-10 overflow-auto rounded-md text-base outline-1 -outline-offset-1 bg-gray-800 outline-white/10 data-leave:transition data-leave:duration-100 data-leave:ease-in data-closed:data-leave:opacity-0 sm:text-sm"
        >
          {taskOptions.map((task) => (
            <ListboxOption
              key={task}
              value={task}
              className="group relative cursor-pointer py-2 pr-9 pl-3 text-white select-none data-focus:bg-indigo-500 data-focus:outline-hidden"
            >
